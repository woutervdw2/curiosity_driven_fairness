...
class BaseLendingEnv(core.FairnessEnv):
  ...
  def _step_impl(self, state, action):
    ...
    #Update UCB reward function with history of actions
    if type(self.reward_fn) == rewards_fn.ScalarDeltaRewardWithUCB:
      self.reward_fn._update_history(action)
    elif type(self.reward_fn) == rewards_fn.ScalarDeltaRewardVisitCounts:
      self.reward_fn._update_history(state, action)
