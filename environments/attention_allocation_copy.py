...
def _get_location_features(params, rng, incidents_occurred):
  ...
  new_array = np.array(shifted_feature_means, dtype='float32')+np.array(feature_noise, dtype='float32')
  return new_array
...
class LocationAllocationEnv(core.FairnessEnv):
  ...
  def __init__(self, params = None, test=False):
    ...
    self.partitions = self.compute_all_partitions()
    self.action_space = spaces.Discrete(len(self.partitions))
    ...
    if params.attention_replacement:
      ...
      # the incidents_seen.
      incidents_seen_space = spaces.MultiDiscrete([100] *
                                                  params.n_locations)
    else:
      incidents_seen_space = spaces.MultiDiscrete(
          [params.n_attention_units + 1] * params.n_locations)

    incidents_reported_space = spaces.MultiDiscrete([100] *
                                                    params.n_locations)
  ...
  def generate_partitions(self,attention_units, n_locations, current_partition, all_partitions):
    ...
    for units_assigned in range(attention_units + 1):
        current_partition.append(units_assigned)
        self.generate_partitions(attention_units - units_assigned, n_locations - 1, current_partition, all_partitions)
        current_partition.pop()

  def compute_all_partitions(self):
    n_locations = self.params.n_locations
    attention_units = self.params.n_attention_units
    all_partitions = []
    self.generate_partitions(attention_units, n_locations, [], all_partitions)
    #Select only partitions with sum of 4 attention units
    all_partitions = [partition for partition in all_partitions if sum(partition) == attention_units]
  ...
  def _state_init(self, rng=None):
    ...
    self.state = State(
        ...
        location_features=np.zeros(
            (n_locations, len(self.initial_params.feature_means)), dtype='float32'))
    self.state_history = []
  ...
  def _step_impl(self, state, action):
    ...
    action = self.partitions[action]
  
    _update_state(state, incidents_occurred, reported_incidents, action)

    if type(self.reward_fn) == rewards.ScalarDeltaRewardWithUCB:
      self.reward_fn._update_history(action)
    elif type(self.reward_fn) == rewards.ScalarDeltaRewardVisitCounts:
      self.reward_fn._update_history(state.incidents_seen, action)
