...
import sys
sys.path.append('code/ml-fairness-gym/')

import attr
import core
import run_util
from agents import rl_agents, allocation_agents
...
def _get_relevant_history(env):
  ...
  relevant_history = np.array([
      np.array([history_item.state.incidents_occurred, 
                env.partitions[history_item.action]])
      for history_item in history
  ])
...
def run(experiment):
  ...
  all_runs_metrics, all_runs_history = zip(*all_runs_metrics)
  
  averaged_metrics = np.average(all_runs_metrics, axis=0).tolist()
  averaged_metrics = averaged_metrics + [np.average(np.array(all_runs_history), axis=0).tolist()]
  metric_names = [
      'discovered_incidents', 'occurred_incidents', 'discovered_occurred_ratio',
      'discovered_occurred_ratio_weighted'
      , 'history'
  ]
